...
{
	long treesize = 0;	// Track size of the tree
	if (subtree != NULL)	// Determine if the tree is valid
	{
		if (subtree->left_ == NULL && subtree->right_ == NULL)	// Determine if the tree is just a leaf
		{
			treesize += 1;	// Add 1 to treesize. With no other subtrees to traverse, there's no need to add anything afterward.
		}
		else if (subtree->left_ != NULL && subtree->right_ != NULL)	// Determine if the tree contains both subtrees
		{
			treesize += 1;	// Count the current node
			treesize += size(subtree->left_);	// Recursively add the size of the left subtree
			treesize += size(subtree->right_);	// Recursively add the size of the right subtree
		}
		else if (subtree->left_ != NULL && subtree->right_ == NULL)	// Determine if the tree contains only the left subtree
		{
			treesize += 1;	// Count the current node
			treesize += size(subtree->left_);	// Recursively add the size of the left subtree
		}
		else if (subtree->left_ == NULL && subtree->right_ != NULL)	// Determine if the tree contains only the right subtree
		{
			treesize += 1;	// Count the current node
			treesize += size(subtree->right_);	// Recursively add the size of the right subtree
		}
	}
	return  treesize;	// Through recursion, treesize has been counted. Just return treesize.
}
...
{
	long leftheight = 0;	// Track the height of the left side
	long rightheight = 0;	// Track the height of the right side
	
	if (subtree != NULL)	// Determine if the tree is valid
	{

		// If the tree is just a leaf, nothing needs to be added

		if (subtree->left_ != NULL && subtree->right_ != NULL)	// Determine if the tree contains both subtrees
		{
			leftheight += 1;	// The left tree exists, so increment height count of the left side
			leftheight += height(subtree->left_);	// Recursively add to the height of the left side
			rightheight += 1;	// The right tree exists, so increment height count of the right side
			rightheight += height(subtree->right_); // Recursively add to the height of the right side
		}
		else if (subtree->left_ != NULL && subtree->right_ == NULL)	// Determine if the tree contains only the left subtree
		{
			leftheight += 1;	// The left tree exists, so increment height count of the left side
	...
	if (leftheight > rightheight)	// Determine if the height of the left side is greatest
	{
		return leftheight;	// Return the height of the left side
	}
	else
	{
		return rightheight;	// Otherwise, return the height of the right side
	}
}
   ...
   BinaryTree:: leaves( const BinaryNode * subtree )
{
	...
	if (subtree != NULL)
	{
		if (subtree->left_ == NULL && subtree->right_ == NULL) // Determine if the tree is just a leaf
		{
			leafcount += 1; // Tree is a leaf. Increment leafcount
		}
		else if (subtree->left_ != NULL && subtree->right_ != NULL)	// Determine if the tree contains both subtrees
		{
			leafcount += leaves(subtree->left_);	// Recursively add the count of leaves on the left subtree
			leafcount += leaves(subtree->right_);	// Recursively add the count of leaves on the right subtree
		}
		else if (subtree->left_ != NULL && subtree->right_ == NULL)	// Determine if the tree contains only the left subtree
		{
			leafcount += leaves(subtree->left_);	// The right tree does not exist. Only have to recursively add the count of leaves on the left subtree
		}
		else if (subtree->left_ == NULL && subtree->right_ != NULL)	// Determine if the tree contains only the right subtree
		{
			leafcount += leaves(subtree->right_);	// The left tree does not exist. Only have to recursively add the count of leaves on the right subtree
		}
	}
	return leafcount;	// All leaves recursively accounted for. Return leafcount
}
...
{
	short LMval = 0;	// Track the value of the leftmost node
	if (subtree != NULL)
	{
		if (subtree->left_ == NULL)	// Determine if the tree does not contain its left subtree
		{
			LMval = subtree->entry_;	// The left subtree does not exist, so the leftmost value is equal to value of the current node 
		}
		else
		{
			LMval = leftmost(subtree->left_);	// The left subtree exists. Recursively find the leftmost value using the left subtree.
		}
	}
	return LMval;	// Leftmost value has been found recursively. Return leftmost value.
}
...
void BinaryTree::postorder(std::vector<short> & traversal, const BinaryNode * subtree)
{
	if (subtree != NULL)	// Determine if the tree is valid
	{
		postorder(traversal, subtree->left_);	// Recursively find the values of the left nodes
		postorder(traversal, subtree->right_);	// Recursively find the values of the right nodes
		traversal.push_back(subtree->entry_);	// Add the value of the current node to the postorder traversal vector
	}
}
